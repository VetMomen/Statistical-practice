...
```{r factor corr}
data.frame(parameterestimates(object = esemfit,standardized = T))[181:195,c(1:4,11)]%>%kable(digits = 3,row.names = F,format = "markdown")
```
...
scalarfit2<-cfa(model = esemmodel,data = big[,-c(1)],group = "Sex",group.equal = c("loadings","intercepts"))
...
measurementInvariance(model=esemmodel,data=big[,-c(1)],strict = T,group="Sex",group.partial = c("O3~1"))
```

Great !! the ESEM Method reached to the same results (partial measure invariance (config. weak, strong, strict, means) ) !! but obviously with better fit measures 
...
|measurement invariance | The Model was good in proving the partial invarince (config , weak , strong , mean) | reached to the same results but with better fit measures ! |
        
        
Finally , ** THIS ANALYSIS REPRESENT A GUIDANCE TO WHAT SUPPOSED TO BE AND PATH WAY FOR COMPAIRING BETWEEN THE TWO METHODS AND NO WAY TO DEPEND ON THE RESULTS HERE IN OUR JUDJEMENT ON BOTH METHODS UNLESS WE INCREASE THE SAMPLE SIZE TO AT LEAST 15-20 SAMPLE FOR eACH PARAMETER **