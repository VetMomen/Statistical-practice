...
library(lavaan);library(semPlot);library(semTools);library(semPower)
#importing the data 
...
dat<-bind_rows(dat2[[1]][-outidx1,],dat2[[2]][-outidx2,])
...
cfacompare<-compareFit(fitB,fitL,fitI,fitFV,fitRP,fitM) #only two variances need to be free
...
esemcompare<-compareFit(esemB,esemL,esemI,esemFV,esemRP,esemM) #only two residuals need to be free 
